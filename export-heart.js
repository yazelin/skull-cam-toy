/* 心跳凸輪版:從 heart.html 的 3D 模型直接產生 1:1 雷切檔(A4,mm)
...
const SHEET = { w: 210, h: 297, m: 6, gap: 3 };   // A4 直放
...
(async () => {
  ...
  <text class="hd" x="${SHEET.m}" y="${SHEET.h - 3}">七夕心跳凸輪 · 3mm 板 · 1:1 · 第 ${i + 1}/${sheets.length} 張 · 黑線=切割 藍字=刻字</text>
${body}
</svg>`;
  });

  pages.forEach((sv, i) => fs.writeFileSync(path.join(OUT, `heart-cut-${i + 1}.svg`), sv));
  ...
  sheets.forEach((s, i) => {
    ...
    d += '0\nENDSEC\n0\nEOF\n';
    fs.writeFileSync(path.join(OUT, `heart-cut-${i + 1}.dxf`), d);
  });
  console.log(`零件種類 ${parts.length} 種,總片數 ${items.length} 片,排成 ${sheets.length} 張 A4`);
  sheets.forEach((s, i) => console.log(`  heart-cut-${i + 1}.svg : ${s.length} 片`));
})();

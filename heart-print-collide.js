/* 心跳凸輪「3D 列印版」的干涉掃描:同 heart-collide.js,多一條規則:同名零件(箱身五片、頂蓋九片)是一體列印的,互碰跳過。
   原始註解:/* 心跳凸輪的干涉掃描:每片板佈 2mm 取樣點,曲柄 36 個角度,兩兩互測「A 的點有沒有落進 B 的實體」。
   當初就是這支抓到相位符號寫反(推桿半個週期陷進凸輪)。
   用法:node heart-print-collide.js      需要本機 HTTP 開著(python3 -m http.server 8099) */
  ...
  const out = await p.evaluate(() => {
    ...
    for (let ai = 0; ai < 36; ai++) {
      ...
      for (let i = 0; i < SOLIDS.length; i++) for (let j = 0; j < SOLIDS.length; j++) {
        if (i === j) continue;
        if (SOLIDS[i].part === SOLIDS[j].part) continue;   // 同件(一體列印)互碰不算干涉
